      ...
      {links.map((link, index) => (
        <li key={index} className="list-none">
          <a
            ...
            className={`bg-white/20 backdrop-blur-md m-0 py-5 px-6 justify-center items-center relative flex transition-all duration-200 sm:py-4 sm:px-4 sm:text-base hover:bg-white/30 hover:backdrop-blur-none !text-white !text-lg font-medium drop-shadow-lg ${index !== links.length - 1 ? "border-b border-white" : ""}`}
          >
            <span className="flex mr-4 sm:mr-3 bg-white/90 rounded-lg p-2 flex-shrink-0">
              <Image alt={`${link.name} logo`} src={link.logoSrc} width={40} height={40} className="w-10 h-10 sm:w-9 sm:h-9" />
            </span>
            <span>{link.name}</span>
          </a>
        </li>
