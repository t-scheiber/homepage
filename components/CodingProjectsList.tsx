...
const CodingProjectsList: React.FC = () => {
  const projects: Project[] = [
    ...
    },
    {
      ...
      url: "https://react-advanced-course-portfolio.thomasscheiber.com/",
      ...
      {projects.map((project, index) => {
        ...
        return (
          <li key={index} className="list-none">
            <div
              className={`bg-white/20 backdrop-blur-md m-0 py-5 px-6 justify-center items-center relative flex transition-all duration-200 sm:py-4 sm:px-4 sm:text-base hover:bg-white/30 hover:backdrop-blur-none !text-white !text-lg font-medium drop-shadow-lg ${index !== projects.length - 1 ? "border-b border-white" : ""}`}
            >
              <a
                href={project.url}
                target="_blank"
                ...
                className="flex items-center flex-1 !text-white !text-lg font-medium drop-shadow-lg"
              >
                <span className="flex mr-4 sm:mb-0 bg-white/90 rounded-lg p-2">
                  <img src={faviconSrc} alt={`${project.name} favicon`} className="w-8 h-8 sm:w-7 sm:h-7" width={32} height={32} />
                </span>
                {project.name}
              </a>
              <a
                ...
                className="ml-4 flex-shrink-0 bg-white/90 rounded-lg p-2"
              >
