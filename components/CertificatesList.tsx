      ...
      {certificates.map(({ href, src, alt, title, organization }, index) => (
        ...
        >
          <a
            ...
            className={`bg-white/20 backdrop-blur-md m-0 py-5 px-6 justify-center items-center relative flex transition-all duration-200 sm:py-4 sm:px-4 sm:text-base sm:flex-col sm:items-start sm:text-left hover:bg-white/30 hover:backdrop-blur-none !text-white !text-lg font-medium drop-shadow-lg ${index !== certificates.length - 1 ? "border-b border-white" : ""}`}
          >
            <span className="flex mr-4 sm:mb-2 bg-white/90 rounded-lg p-2">
